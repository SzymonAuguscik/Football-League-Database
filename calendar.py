...
scores = [0 for i in range(9)] + [1 for i in range(13)] + [2 for i in range(12)] + [3 for i in range(7)] + \
         [4 for i in range(4)] + [5 for i in range(2)] + [6 for i in range(1)]  # chances of scoring n goals
...
def main():
    ...
    for i in range(len(table)):
        for j in range(0, len(table[i]), 2):
            ...
            file.write("INSERT INTO games VALUES(" + str(count) + ",'" + teams[table[i][j]] + "','" + teams[
                table[i][j + 1]] + "','{" + str(randomScore(scores)) + "," + str(
                randomScore(scores)) + "}'," + date + ");\n")
            count += 1
    ...
    for i in range(len(table)):
        for j in range(0, len(table[i]), 2):
            ...
            file.write("INSERT INTO games VALUES(" + str(count) + ",'" + teams[table[i][j + 1]] + "','" + teams[
                table[i][j]] + "','{" + str(randomScore(scores)) + "," + str(
                randomScore(scores)) + "}'," + date + ");\n")
            count += 1
        startingDay = dateUpdate(startingDay, 7)
